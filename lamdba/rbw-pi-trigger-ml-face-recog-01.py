...
def lambda_handler(event, context):
    logger.debug("RBW> Start rbw-pi-trigger-ml-face-recog-01--------------------------------------------------")
    ...
    logger.debug("RBW>  /End rbw-pi-trigger-ml-face-recog-01")



def call_rekognition(bucket='bucket', filename='input.jpg'):
    logger.debug("RBW>  ===== Start call_rekognition(bucket=%s', filename=%s)" %(bucket, filename))
    ...
    logger.debug("RBW>  /End call_rekognition()")
    

def face_compare(bucket='bucket', filename='input.jpg'):   
    logger.debug("RBW> ==== Start face_compare(bucket=%s', filename=%s)" %(bucket, filename))
    ...
    for name in os.environ['REKOG_NAME_LIST'].split(','):
        ...
        logger.debug("RBW> Checking similarity with '%s'" % (ref_img))
        ...
        logger.debug("RBW> Similarity response |%s|" % (response))

        for faceMatch in response['FaceMatches']:
            position = faceMatch['Face']['BoundingBox']
            confidence = str(faceMatch['Face']['Confidence'])
            logger.debug('RBW> The face at ' +
                str(position['Left']) + ' ' +
                str(position['Top']) +
    ...
    logger.debug("RBW>  /End face_compare(): "+ret_str)
    return ret_str


# https://boto3.amazonaws.com/v1/documentation/api/latest/guide/sqs-example-sending-receiving-msgs.html
def push_to_generate_mp3(text="nothing to say"):
    logger.debug("RBW> ===== Start push_to_generate_mp3(text=%s')" %(text))
    
    queue_url = os.environ['SQS_URL_GENERATEMP3']
    logger.debug("RBW> Pushing to: %s')" %(queue_url))
    ...
    logger.debug("RBW>  Message sent.  ID: "+response['MessageId'])
    logger.debug("RBW>  /End push_to_generate_mp3()")
